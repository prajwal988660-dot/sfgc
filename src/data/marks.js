...
import { CLASSES, findClassByStudent } from './attendance.js'
...
// Pure: compute a student's full marks summary from a records object.
export function computeStudentMarks(records, studentId) {
  const found = findClassByStudent(studentId)
  if (!found) return null
  const { cls, student } = found

  const subjects = cls.subjects.map((name) => {
    const m = records?.[cls.id]?.[name]?.[student.id] || { ia1: 0, ia2: 0, assignment: 0 }
