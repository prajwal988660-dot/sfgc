...
import { MIN_ATTENDANCE, seedRecords } from '../data/attendance.js'
import { isSupabaseEnabled } from '../lib/supabase.js'
import {
  fetchAttendance, upsertAttendanceRows, rowsToAttendance, attendanceToRows,
  fetchSessions, insertSession,
} from '../lib/db.js'
import { useSubjects } from './SubjectsContext.jsx'
import { useClasses } from './ClassesContext.jsx'
...
export function AttendanceProvider({ children }) {
  ...
  const { getClass, findStudent } = useClasses()
  ...
  const markSession = ({ classId, subject, date, time, presentIds, teacherId, teacherName }) => {
    ...
    const entry = {
      ...
      classId, classLabel: cls.label, subject, date, time: time || '',
  ...
  const getStudentSummary = (rawId) => {
    const found = findStudent(rawId)
    if (!found) return null
    const { cls, student } = found
    const names = getSubjects(cls.id)
    ...
    return {
      ...
      program: cls.program || cls.stream, semester: cls.semester || cls.year, section: cls.section,
