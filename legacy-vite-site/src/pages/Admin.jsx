    ...
    <>
      ...
      <section className="section">
        <div className="container">
          ...
          {view === 'classes' ? (
            <ClassesView showFlash={showFlash} />
          ) : view === 'regs' ? (
            <RegistrationsView
              events={events}
...
const STREAMS = ['B.Com', 'B.Com with BDA', 'BCA', 'BBA', 'BBA Aviation', 'BSc BBG', 'BSc EMC', 'M.Com', 'MBA', 'MCA', 'Global MBA']
const YEARS = ['I', 'II', 'III']
const SECTIONS = ['A', 'B', 'C']

function ClassesView({ showFlash }) {
  const { classes, addClass, removeClass, addStudent, removeStudent } = useClasses()
  ...
  const handleAddClass = () => {
    const res = addClass({ stream, year, section })
    if (res.error) { showFlash('⚠️ ' + res.error); return }
    showFlash(`✅ Added class ${res.label}.`)
    setExpanded(res.id)
  }
  const handleAddStudent = (classId) => {
    const name = (nameInputs[classId] || '').trim()
    if (!name) return
    const res = addStudent(classId, name)
    if (res.error) { showFlash('⚠️ ' + res.error); return }
    showFlash(`✅ Added ${name} — ID ${res.id}.`)
    setNameInputs((p) => ({ ...p, [classId]: '' }))
  }
  const handleRemoveClass = (c) => {
    if (window.confirm(`Delete class ${c.label} and its ${c.students.length} student(s)?`)) { removeClass(c.id); showFlash('🗑️ Class removed.') }
  }
  const handleRemoveStudent = (classId, s) => {
    if (window.confirm(`Remove ${s.name} (${s.id})?`)) { removeStudent(classId, s.id); showFlash('🗑️ Student removed.') }
  }

  return (
    <div className="admin-grid">
      {/* Add class */}
      <form className="admin-form card" onSubmit={(e) => { e.preventDefault(); handleAddClass() }}>
        <div className="admin-form__head"><h2>Add Class</h2></div>
        <div className="form-grid">
          <div className="form-row"><label>Stream</label><select value={stream} onChange={(e) => setStream(e.target.value)}>{STREAMS.map((s) => <option key={s}>{s}</option>)}</select></div>
          <div className="form-row"><label>Year</label><select value={year} onChange={(e) => setYear(e.target.value)}>{YEARS.map((y) => <option key={y}>{y}</option>)}</select></div>
        </div>
        <div className="form-row"><label>Section</label><select value={section} onChange={(e) => setSection(e.target.value)}>{SECTIONS.map((s) => <option key={s}>Section {s}</option>)}</select></div>
        <button type="submit" className="btn btn--primary" style={{ width: '100%', justifyContent: 'center' }}>＋ Add Class</button>
        <p className="admin-note">Add students inside each class — every student gets a unique ID automatically, and the roster is kept in alphabetical order.</p>
      </form>

      {/* Classes list */}
      <div className="admin-list">
        <div className="admin-list__head"><h2>Classes <span className="admin-count">{classes.length}</span></h2></div>
        {classes.map((c) => (
          <div className="cls-card" key={c.id}>
            <div className="cls-card__head">
              <button className="cls-card__title" onClick={() => setExpanded(expanded === c.id ? null : c.id)}>
                <span className="chev" style={{ transform: expanded === c.id ? 'rotate(90deg)' : 'none' }}>▸</span>
                <div>
                  <div className="admin-item__title">{c.label}</div>
                  <div className="admin-item__meta">{c.stream} · Year {c.year} · Sec {c.section} · {c.students.length} student{c.students.length !== 1 ? 's' : ''}</div>
                </div>
              </button>
              <button className="admin-btn admin-btn--del" onClick={() => handleRemoveClass(c)}>Delete</button>
            </div>
              ...
              <div className="cls-students">
                <div className="cls-add">
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddStudent(c.id))}
                    placeholder="Student full name"
                  />
                  <button className="btn btn--primary btn--sm" onClick={() => handleAddStudent(c.id)}>Add Student</button>
                </div>
                {c.students.length > 0 ? (
                  <div className="table-scroll">
                    <table className="data-table">
                      <thead><tr><th>Roll</th><th>Name</th><th>Unique ID</th><th></th></tr></thead>
                      <tbody>
                        {c.students.map((s) => (
                          <tr key={s.id}>
                            <td>{s.roll}</td>
                            <td className="sub">{s.photo} {s.name}</td>
                            <td className="regs-ticket">{s.id}</td>
                            <td><button className="admin-btn admin-btn--del" onClick={() => handleRemoveStudent(c.id, s)}>×</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div style={{ color: 'var(--muted)', fontSize: '0.85rem', padding: '6px 0' }}>No students yet — add one above.</div>
                )}
              </div>
            )}
          </div>
