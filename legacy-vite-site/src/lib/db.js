...
export async function fetchSessions() {
  ...
  return error ? null : data.map((s) => ({
    id: s.id, classId: s.class_id, classLabel: s.class_label, subject: s.subject, date: s.date, time: s.time,
...
export async function insertSession(s) {
  ...
  const { error } = await supabase.from('attendance_sessions').insert({
    id: s.id, class_id: s.classId, class_label: s.classLabel, subject: s.subject, date: s.date, time: s.time,
...
export async function fetchSubjects() {
  ...
  warn('fetchSubjects', error)
  return error ? null : data
}
export async function seedSubjects(rows) {
  if (!isSupabaseEnabled || !rows.length) return
  const { error } = await supabase.from('class_subjects').upsert(rows)
  warn('seedSubjects', error)
}
export async function insertSubject(classId, subject) {
  if (!isSupabaseEnabled) return
  const { error } = await supabase.from('class_subjects').upsert({ class_id: classId, subject })
  warn('insertSubject', error)
}
export async function deleteSubjectRow(classId, subject) {
  if (!isSupabaseEnabled) return
  const { error } = await supabase.from('class_subjects').delete().eq('class_id', classId).eq('subject', subject)
  warn('deleteSubject', error)
}
